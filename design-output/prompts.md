# AnemiaScan mockup prompts

Generated with the built-in image generation tool.

## Main board

Use case: ui-mockup. Create a polished high-resolution UI design contact sheet with EXACTLY SIX full portrait Android screens in a 3-column by 2-row grid. Each screen proportions 412:915, flat UI no perspective, no hardware bezels, complete unclipped screen. Tiny external labels. Maximize resolution and screen readability. This is board 1 main flow. Six panels in reading order: Home; Capture anemia; Analyzing; Fresh Moderate anemia result; History populated; Sample photos. Follow the complete attached design brief below for these six screens. Other variants will be rendered on a separate board so don't squeeze alternate states into these panels. Premium Google Material 3 precision, spacious but ensure ALL required elements fit. Palette primary #006B73, primary-container #D5F2EF, surface #F8FAF8, surface-variant #EAF0ED, outline #71817E, text #162E2C. Figtree and Hind Siliguri styled humanist typography, Bangla one step larger paired with English for all key labels and instructions, careful authentic Bangla. Card radius 24dp, buttons pills height56dp, 24dp page padding, 8/16/24 rhythm. Rounded Material icons. No decorative gradients. Camera background photoreal closeup eyelid, restrained dark overlay with instructional translucent line drawing finger lowering eyelid. Risk colors green #236A44, amber #8A5700, coral #B13D38 only risk badges; saved confirmation pale teal to obey risk-only color rule. On moderate result only active amber segment colored, other risk segments neutral with labels and active icon so colorblind safe. Demo photos visually representative tiny crops, not certified actual medical reference images. All text readable and impeccably typeset, avoid filler. Exact requirements:
